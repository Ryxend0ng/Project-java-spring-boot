...
<div class="main-content">
		<form action="/admin/list-roles" id="formSubmit"
			method="get">
									...
									<div class="pull-right tableTools-container">
										<div class="dt-buttons btn-overlap btn-group">
											<a flag="info"
												...
												href='<c:url value="/admin/add-roles"/>'> <span>
										...
										<table class="table table-bordered table-striped ">
											...
											<tbody>
												<c:forEach items="${roles}" var="roles">
													<tr>
														<td><input type="checkbox" id="checkbox_${roles.id}"
															value="${roles.id}" name="ids"></td>
														<td>${roles.id }</td>
													
										        		<td>${roles.name }</td>
										        		<td class="">${roles.code }</td>
										        		<td c>${roles.status }</td>
														<td>
															 <a class="btn btn-sm btn-primary btn-edit"
															data-toggle="tooltip" title="Cập nhật bài viết"
															href="${base}/admin/edit-roles/${roles.code}"><i class="fa fa-pencil-square-o"
																aria-hidden="true"></i> </a>
																<button class="btn btn-sm btn-danger btn-edit"
															data-toggle="tooltip" title="Xóa  bài viết"
															onclick="deleteNew(${roles.id })" ><i class="fa fa-trash-o bigger-110 "></i> </button></td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
			...
			function deleteNew(data) {
		        ...
		        $.ajax({
		            ...
		            success: function (jsonResult) {
		            	alert("Chúc mừng!!! Đã xóa thành công Quyền  có ID là: "+jsonResult.message);
		            	window.location.href = "/admin/list-roles?page=1&maxPageItem=2";
		            },
