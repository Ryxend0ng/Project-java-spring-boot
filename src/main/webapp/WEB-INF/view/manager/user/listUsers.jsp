...
<div class="main-content">
		<form action="/admin/list-users" id="formSubmit"
			method="get">
									...
									<div class="pull-right tableTools-container">
										<div class="dt-buttons btn-overlap btn-group">
											<a flag="info"
												...
												href='<c:url value="/admin/add-users"/>'> <span>
										...
										<table class="table table-bordered table-striped ">
											...
											<tbody>
												<c:forEach items="${users}" var="users">
													<tr>
														<td><input type="checkbox" id="checkbox_${users.id}"
															value="${users.id}" name="ids"></td>
														<td>${users.id }</td>
													
										        		<td>${users.userName }</td>
										        		<td class="">${users.email }</td>
										        		<td c>${users.status }</td>
														<td>
															 <a class="btn btn-sm btn-primary btn-edit"
															data-toggle="tooltip" title="Cập nhật bài viết"
															href="${base}/admin/edit-users/${users.userName}"><i class="fa fa-pencil-square-o"
																aria-hidden="true"></i> </a>
																<button class="btn btn-sm btn-danger btn-edit"
															data-toggle="tooltip" title="Xóa  bài viết"
															onclick="deleteNew(${users.id })" ><i class="fa fa-trash-o bigger-110 "></i> </button></td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
			...
			function deleteNew(data) {
		        ...
		        $.ajax({
		            ...
		            success: function (jsonResult) {
		            	alert("Chúc mừng!!! Đã xóa thành công người dùng có ID là: "+jsonResult.message);
		            	window.location.href = "/admin/list-users?page=1&maxPageItem=2";
		            },
